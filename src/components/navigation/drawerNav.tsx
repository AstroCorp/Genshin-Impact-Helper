...
const DrawerNav = ({ descriptors, state, navigation }: DrawerContentComponentProps) => (
			...
			{state.routes.map((route, i) => {
				...
				return (
					<TouchableNativeFeedback
						key={route.name}
						onPress={() => {
							const action = focused ? DrawerActions.closeDrawer() : CommonActions.navigate({ name: route.name, merge: true });
